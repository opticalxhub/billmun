...
export async function GET() {
  ...
  const { data } = await supabaseAdmin
    ...
    .eq("committee_id", context.committee_id)
...
export async function POST(req: NextRequest) {
  const { context, error, status } = await getRequestUserContext();
  if (!context) return NextResponse.json({ error }, { status: status || 500 });
  if (!context.committee_id) return NextResponse.json({ error: "No committee context" }, { status: 403 });
  ...
  if (action === "update_status") {
    ...
    const { data: existing } = await supabaseAdmin
      ...
      .eq("id", id)
      .maybeSingle();
    if (!existing?.id || existing.committee_id !== context.committee_id) {  
      return NextResponse.json({ error: "Forbidden" }, { status: 403 });
    }
